...
async function loadStatePage(){const b=se('stateRefresh');if(b){b.disabled=true;b.textContent='…'};try{const now=Date.now(),from=now-6*3600000,d=await stateApi({api:'state',from:Math.floor(from/1000),to:Math.floor(now/1000),bucket:300});const rows=(Array.isArray(d)?d:[]).map(x=>({ts:epochMs(x.bucket_ts),hr:sn(x.avg_hr),motion:sn(x.motion_score),rrCount:Number(x.rr_count||0),rmssd:sn(x.rmssd_ms),sleep:x.sleep_state==='SLEEP'})).filter(x=>x.ts).sort((a,b)=>a.ts-b.ts);render(classify(rows));if(b)b.textContent='✓'}catch(e){render({kind:'unknown',row:null,reason:'Помилка оновлення: '+e.message});if(b)b.textContent='!'}finally{setTimeout(()=>{if(b){b.disabled=false;b.textContent='↻'}},900)}}
